...
const int jhdLen = strlen(JHEADER); // Calculate the length of the JPEG HTTP header
// ===== rtos task handles =========================
...
void handleCapture() {
  ...
  cam.run(); // Capture a single frame
  client.write(JHEADER, jhdLen); // Send JPEG header
  client.write((char*)cam.getfb(), cam.getSize()); // Send the captured frame
}
...
// }
void mjpegCB(void* pvParameters) {
  server.on("/capture", HTTP_GET, handleCapture); // Register new URL for capturing single frame
  server.onNotFound(handleNotFound);
  server.begin();
  for (;;) {
    server.handleClient();
...
void setup()
{
  ...
  WiFi.mode(WIFI_STA);
  WiFi.begin("SkyFrost702", "marken885");//WIFI名称和密码
